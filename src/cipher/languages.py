...
LANGUAGE_CORPORA: dict[str, list[tuple[str, str]]] = {
    "french": [
        ...
        ("Le Comte de Monte-Cristo", "https://www.gutenberg.org/cache/epub/17989/pg17989.txt"),
    ...
    "german": [
        ...
        ("Also sprach Zarathustra", "https://www.gutenberg.org/cache/epub/7205/pg7205.txt"),
    ],
    "spanish": [
    ...
    for m in ["*** START OF THE PROJECT GUTENBERG", "*** START OF THIS PROJECT GUTENBERG"]:
        ...
        if idx != -1:
            ...
            if nl != -1:
                text = text[nl + 1:]
            break
    for m in ["*** END OF THE PROJECT GUTENBERG", "*** END OF THIS PROJECT GUTENBERG",
...
def download_language_corpus(language: str, force: bool = False) -> str:
    ...
    if not urls:
        raise ValueError(f"Unknown language: {language}. Available: {list(LANGUAGE_CORPORA)}")

    DATA_DIR.mkdir(parents=True, exist_ok=True)
    parts: list[str] = []

    with Progress() as progress:
        task = progress.add_task(f"[cyan]Downloading {language} corpus…", total=len(urls))
    ...
    cleaned = "".join(ch for ch in full_text if ch in string.ascii_lowercase or ch == " ")
...
LANGUAGE_FREQ: dict[str, np.ndarray] = {
    ...
    "french": np.array([
        0.07636, 0.00901, 0.03260, 0.03669, 0.14715, 0.01066, 0.00866,
        0.00737, 0.07529, 0.00613, 0.00049, 0.05456, 0.02968, 0.07095,
        0.05796, 0.02521, 0.01362, 0.06553, 0.07948, 0.07244, 0.06311,
        0.01838, 0.00074, 0.00427, 0.00128, 0.00326,
    ]),
    "german": np.array([
    ...
    "spanish": np.array([
        0.11525, 0.02215, 0.04019, 0.05010, 0.12181, 0.00692, 0.01768,
